...
def results():
    ...
    if "travel_dates" not in planner.user_data or "parks" not in planner.user_data:
        ...
        flash(
            "Missing travel information. Please start the planning process again.",
            "error",
        )
    ...
    session["booking_dates"] = [
        date.isoformat() if date else None for date in booking_dates
    ]
...
def download_pdf():
    ...
    for key, value in milestones_str.items():
        if isinstance(value, str):
            ...
            milestones[key] = [
                parse_date(date_str) if date_str else None for date_str in value
            ]
        else:
    ...
    # Title
    elements.append(
        Paragraph("Disney World Lightning Lane Plan", styles["DisneyTitle"])
    )
    elements.append(
        Paragraph(
            f"Generated on {datetime.date.today().strftime('%A, %B %d, %Y')}",
            styles["Normal"],
        )
    )
    ...
    data = [
        [
            "Party Size:",
            f"{user_data.get('adults', 0)} adults"
            + (
                f", {user_data.get('children', 0)} children"
                if user_data.get("children", 0) > 0
                else ""
            ),
        ],
    ...
    # Park Visit Schedule
    if (
        "travel_dates" in user_data
        and user_data["travel_dates"]
        and "parks" in user_data
    ):
        elements.append(Paragraph("Park Visit Schedule", styles["DisneyHeading3"]))

        visit_data = [["Date", "Park"]]
        for i, (date, park) in enumerate(
            zip(user_data.get("travel_dates", []), user_data.get("parks", []))
        ):
            if date and hasattr(
                date, "strftime"
    ...
    elements.append(
        Paragraph("Lightning Lane Booking Information", styles["DisneyHeading3"])
    )

    if (
        user_data.get("resort_guest", False)
        and booking_date
        and hasattr(booking_date, "strftime")
    ):
        elements.append(
            Paragraph(
                "As a Disney Resort guest, you can book Lightning Lane passes for your entire stay starting at 7:00 AM ET on:",
                styles["Normal"],
            )
        )
        elements.append(
            Paragraph(booking_date.strftime("%A, %B %d, %Y"), styles["Normal"])
        )
    elif user_data.get("travel_dates") and len(user_data.get("travel_dates", [])) > 0:
        elements.append(
            Paragraph(
                "As an off-site guest, you can book Lightning Lane passes starting 3 days before each park visit:",
                styles["Normal"],
            )
        )
        ...
        for i, (visit_date, park) in enumerate(
            zip(user_data.get("travel_dates", []), user_data.get("parks", []))
        ):
            if hasattr(visit_date, "strftime"):
                ...
                if (
                    i < len(booking_dates)
                    and booking_dates[i]
                    and hasattr(booking_dates[i], "strftime")
                ):
    ...
    if milestones:
        ...
        for label, date_value in milestones.items():
            if isinstance(date_value, (datetime.date, datetime.datetime)) and hasattr(
                date_value, "strftime"
            ):
    ...
    for scenario_key, scenario in scenarios.items():
        ...
        for daily in scenario["daily"]:
            ...
            single_passes = (
                ", ".join(daily.get("single_passes", []))
                if daily.get("single_passes")
                else "None"
            )
    ...
    for park, park_data in park_recommendations.items():
        ...
        elements.append(
            Paragraph("Recommended Lightning Lane Selections:", styles["Normal"])
        )
            ...
            for attraction in park_data.get("recommended", {}).get("tier1", []):
                elements.append(
                    Paragraph(
                        f"• {attraction}",
                        ParagraphStyle(
                            "Bullet", parent=styles["Normal"], leftIndent=20
                        ),
                    )
                )
        ...
        # Single passes
        if user_data.get("include_single_pass", False) and park_data.get(
            "single_passes", []
        ):
            elements.append(
                Paragraph(
                    "Your Selected Single Passes:",
                    ParagraphStyle(
                        "BulletHeading",
                        parent=styles["Normal"],
                        fontName="Helvetica-Bold",
            ...
            for pass_info in park_data.get("single_passes", []):
                elements.append(
                    Paragraph(
                        f"• {pass_info.get('name', '')} (${pass_info.get('min', 0)}-${pass_info.get('max', 0)} per person)",
                        ParagraphStyle(
                            "Bullet", parent=styles["Normal"], leftIndent=20
                        ),
                    )
        ...
        for tip in park_data.get("tips", []):
            elements.append(
                Paragraph(
                    f"• {tip}",
                    ParagraphStyle("Bullet", parent=styles["Normal"], leftIndent=20),
                )
            )
    ...
    elements.append(
        Paragraph(
            "Note: This is an unofficial planning tool and is not affiliated with Walt Disney World Resort or the Walt Disney Company.",
            ParagraphStyle(
    ...
    elements.append(
        Paragraph(
            "All prices shown are estimates based on historical and seasonal data. Actual costs may vary.",
            ParagraphStyle(
                "Footer", parent=styles["Normal"], fontSize=8, textColor=colors.grey
            ),
        )
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=Disney_Lightning_Lane_Plan.pdf"
    )
...
def download_excel():
    ...
    header_fill = PatternFill(
        start_color="0078D2", end_color="0078D2", fill_type="solid"
    )
    ...
    ws["A4"] = "Party Size:"
    ws["B4"] = f"{user_data.get('adults', 0)} adults" + (
        f", {user_data.get('children', 0)} children"
        if user_data.get("children", 0) > 0
        else ""
    )
    ...
    for scenario_key, scenario in scenarios.items():
        ...
        for daily in scenario["daily"]:
            if isinstance(daily["date"], str):
                date_parts = daily["date"].split(",")[
                    0
                ]  # Get just the day part, e.g. "Monday, April 15" -> "Monday"
            else:
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=Disney_Lightning_Lane_Plan.xlsx"
    )
