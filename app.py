...
def user_info():
    if request.method == 'POST':
        ...
        planner.user_data["number_of_people"] = planner.user_data["adults"] + planner.user_data["children"]
        
        # Process Lightning Lane preferences
        planner.user_data["include_single_pass"] = request.form.get('include_single_pass') == 'yes'
        planner.user_data["include_premier"] = request.form.get('include_premier') == 'yes'
...
def results():
    ...
    for i, park in enumerate(planner.user_data["parks"]):
        ...
        if planner.user_data["include_premier"]:
            ...
            premier_info = {
                "min": premier_cost["min"],
                "max": premier_cost["max"]
            }
