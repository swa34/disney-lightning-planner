...
class DisneyLightningLanePlanner:
    def __init__(self):
        ...
        self.single_pass_costs = {
            ...
            "EPCOT": {
                "Guardians of the Galaxy: Cosmic Rewind": {"min": 16, "max": 19}
            },
            "Hollywood Studios": {
            ...
            "Animal Kingdom": {
                "Avatar Flight of Passage": {"min": 15, "max": 18}
            }
        }
        ...
        self.recommended_attractions = {
            ...
            "EPCOT": {
                ...
                "Tier 2": ["Soarin'", "Mission: SPACE", "Test Track (reopening summer 2025)"]
            },
            "Hollywood Studios": {
                "Tier 1": ["Slinky Dog Dash"],
                "Tier 2": ["Millennium Falcon: Smugglers Run", "Toy Story Mania", "Tower of Terror"]
            },
            "Animal Kingdom": {
                # No tiers for Animal Kingdom
                "Recommended": ["Na'vi River Journey", "Kilimanjaro Safaris", "Expedition Everest"]
            }
        }
    ...
    def calculate_costs(self):
        ...
        scenarios = {
            "scenario2": {"name": "Multi Pass Only (Budget Option)", "total_min": 0, "total_max": 0, "daily": []}
        }
        
        # Add scenario 1 if user wants Single Pass
        if self.user_data["include_single_pass"]:
            scenarios["scenario1"] = {"name": "Multi Pass + Selected Single Passes", "total_min": 0, "total_max": 0, "daily": []}
        
        # Add scenario 3 if user wants Premier Pass
        if self.user_data["include_premier"]:
            scenarios["scenario3"] = {"name": "Premier Pass (Premium Option)", "total_min": 0, "total_max": 0, "daily": []}
        ...
        scenarios["scenario4"] = {"name": strategy_name, "total_min": 0, "total_max": 0, "daily": []}
        ...
        for i, park in enumerate(self.user_data["parks"]):
            ...
            multi_min = self.multi_pass_costs[park][f"{price_key_suffix}min" if price_key_suffix else "min"]
            multi_max = self.multi_pass_costs[park][f"{price_key_suffix}max" if price_key_suffix else "max"]
            ...
            if self.user_data["include_premier"] and "scenario3" in scenarios:
                premier_min = self.premier_pass_costs[park]["min"] * self.user_data["adults"]
                premier_max = self.premier_pass_costs[park]["max"] * self.user_data["adults"]
        ...
        for i, park in enumerate(self.user_data["parks"]):
            ...
            if park in ["Magic Kingdom", "Hollywood Studios"]:
                ...
                if self.user_data["include_single_pass"]:
                    user_selected = self.user_data["selected_single_passes"].get(park, [])
            ...
            scenarios["scenario4"]["daily"].append({
                ...
                "min": daily_min,
                "max": daily_max,
                "single_passes": selected_passes if self.user_data["include_single_pass"] else []
            })
